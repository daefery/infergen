...
| E0.2 | CLI Skeleton & Config | Planned | `telemetra` CLI with `init`, `scan`, `generate`, `check`, `watch` command stubs. Config schema (`telemetra.config.*`) + loader. Framework/language auto-detection on `init`. | Depends on E0.1. |
| E0.3 | TS/JS AST Parser Integration | Planned | Integrate SWC/OXC to parse TypeScript/JavaScript into a normalized AST the scan engine consumes. Define the internal AST abstraction so other languages plug in later. | Depends on E0.1. Pick SWC vs OXC early. |
...
| Epic No | Name | Status | Description | Note/Blocker/Dependency |
|---|---|---|---|---|
| E1.1 | Event Catalog Schema & Store | Planned | Define `catalog.yaml` schema: event name, trigger provenance, properties (name/type/required/PII), providers, status, confidence, stable ID. Read/write/serialize with stable, diff-friendly ordering. | Depends on E0.4. |
| E1.2 | Heuristic Event Namer | Planned | Derive event names + property candidates from identifiers (components, routes, handlers). Apply naming convention. Assign confidence scores. | Depends on E1.1. |
...
| Epic No | Name | Status | Description | Note/Blocker/Dependency |
|---|---|---|---|---|
| E2.1 | TypeScript Codegen | Planned | Generate a typed SDK from the catalog: one strongly-typed function/key per event, property shapes enforced at compile time. Deterministic, idempotent output. | Depends on E1.1. |
| E2.2 | Generated SDK Ergonomics | Planned | Autocomplete-friendly API, JSDoc from catalog descriptions, tree-shakeable, framework-friendly imports. | Depends on E2.1. |
...
| Epic No | Name | Status | Description | Note/Blocker/Dependency |
|---|---|---|---|---|
| E3.1 | Provider Plugin Interface | Planned | Define the provider contract (identify, track, flush, shutdown). Plugin registration + config. | Depends on E2.1. |
| E3.2 | First-Party Provider Adapters | Planned | Implement Segment, Amplitude, Mixpanel, PostHog, GA4, RudderStack, and a generic HTTP webhook provider. | Depends on E3.1. |
...
Goal: make Telemetra pleasant, visible, and extensible.
